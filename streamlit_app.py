...
def main():
    ...
    if page_selection == "자유문서 아카이브":
        ...
        action = st.radio("문서 관리", ["문서 읽기", "문서 수정", "문서 삭제", "문서 생성"] if st.session_state.logged_in else ["문서 읽기"])

        if action == "문서 생성" and st.session_state.logged_in:
        ...
        elif action == "문서 수정" and st.session_state.logged_in:
            st.subheader("문서 수정")
            pages = get_all_pages()
            if pages:
        ...
        elif action == "문서 삭제" and st.session_state.logged_in:
            st.subheader("문서 삭제")
            pages = get_all_pages()
            if pages:
                page_titles = [page[0] for page in pages]
